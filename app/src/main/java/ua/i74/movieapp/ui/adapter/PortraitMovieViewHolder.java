...
public class PortraitMovieViewHolder extends RecyclerView.ViewHolder {
    ...
    public void bindModel(MovieModel movie) {
        if (position >= 0 && listener != null) {
            ...
            if (movie.getGenreList() == null || movie.getGenreList().isEmpty())
                ...
                Picasso.get()
                        .load(ServiceData.POSTER_IMAGE_URL + movie.getPosterPath())
                        .centerInside()
                        .error(R.drawable.emoticon_devil_outline)
                        .networkPolicy(NetworkPolicy.NO_CACHE)
                        .resizeDimen(R.dimen.listitem_poster_width, R.dimen.listitem_poster_height)
